    ...
    // To send notification when the contract receives fund
     receive() external payable {
        IPUSHCommInterface(EPNS_COMM_ADDRESS).sendNotification(
            ...
            address(msg.sender), // to recipient, put address(this) in case you want Broadcast or Subset. For Targetted put the address to which you want to send
            bytes(
                string(
                    ...
                    abi.encodePacked(
                        ...
                        "+", // segregator
                        "Notification", // this is notificaiton title
                        "+", // segregator
                        "PUSH Received" // notification body
                    )
                )
            )
        );
